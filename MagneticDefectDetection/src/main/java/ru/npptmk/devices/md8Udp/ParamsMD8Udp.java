...
public class ParamsMD8Udp implements Serializable {
    private static final long serialVersionUID = 5879399746862556764L;
    ...
    /**
     * Создаются параметры с усилением во всех каналах 20, порогом 50, и
     * смещением каналов по оси трубы на четных каналах - 230 мм, а на нечетных
     * - 290. Фильтр 0;
     *
    ...
    /**
     * масштаб перевода значений графика к значениям порога, цифра определена
     * визуально по блоку Сапфир
     */
    ...
    public ParamsMD8Udp getClone() {
        ParamsMD8Udp clone = new ParamsMD8Udp();
        clone.gain = Arrays.copyOf(gain, gain.length);
        clone.offset = Arrays.copyOf(offset, offset.length);
        clone.porog = Arrays.copyOf(porog, porog.length);
        clone.filtr = filtr;
        return clone;
    }
