...
/**
 * Диалог для редактирования параметров самописца дефектоскопа.
 *
 * @author MalginAS
 */
public class DfPanelParamDialog extends javax.swing.JDialog {

    // Драйверы установок дефектоскопии.
    private final DeviceMD8Udp drvMD;
    private final DeviceUSKUdp drvUSK1;
    private final DeviceUSKUdp drvUSK2;
    private final SamopPnlDfParams par;
     ...
     *
     * @param drvMD драйвер магнитки.
     * @param drvUSK1 дпайвер первого блока УЗК.
     * @param drvUSK2 драйвер второго блока УЗК.
     * @param par Редактируемые параметры.
     */
    public DfPanelParamDialog(DeviceMD8Udp drvMD, DeviceUSKUdp drvUSK1, DeviceUSKUdp drvUSK2, SamopPnlDfParams par) {
        ...
        isChanging = true;
        initComponents();
        cbUnit.addItem("Блок МД");
        if (par.devId == Devicess.ID_MD) {
    ...
    private void setName() {
        ...
        par.name = "Дефектоскоп " + cbUnit.getSelectedItem() + " " + cbChannel.getSelectedItem();
        edName.setText(par.name);
    }
    ...
    private void setUnit() {
        ...
        if (par.devId == Devicess.ID_MD) {
            ...
            for (int i = 0; i < 8; i++) {
                if (cd.getParams().prms[i].getHardware_type() == DeviceUSKUdpParam.FLAW_CONTROL) {
                    cbChannel.addItem("Канал " + (i + 1));
                }
                if (par.chanId == i) {
    ...
    private void initComponents() {
        ...
        setTitle("Параметры самописца дефектоскопа.");
