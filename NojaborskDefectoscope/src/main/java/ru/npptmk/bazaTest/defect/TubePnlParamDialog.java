 ...
 * @author MalginAS
 */
public class TubePnlParamDialog extends javax.swing.JDialog implements ListSelectionListener {

    private final TubeGrphParams par;
     ...
     */
    public TubePnlParamDialog(DeviceMD8Udp md, DeviceUSKUdp usk1, DeviceUSKUdp usk2, TubeGrphParams par) {
        ...
        oldColors = Arrays.copyOf(par.clrs, par.clrs.length);
        oldUnits = Arrays.copyOf(par.devIds, par.devIds.length);
        oldChans = Arrays.copyOf(par.cahsnIds, par.cahsnIds.length);
        initComponents();
        oldName = "Отображение трубы";
        ...
        for (int i = 0; i < par.names.length; i++) {
            mod.addElement(new ListGrItem(par.names[i], par.clrs[i]));
        }
    }
    ...
    private void initComponents() {
        ...
        lsGrafs.setFont(new java.awt.Font("Segoe UI", 0, 18)); // NOI18N
        ...
        btRemove.setFont(new java.awt.Font("Segoe UI", 0, 18)); // NOI18N
        ...
        btColor.setFont(new java.awt.Font("Segoe UI", 0, 18)); // NOI18N
    ...
    private void btColorActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btColorActionPerformed
        ...
        Color newCol = JColorChooser.showDialog(null, "Выберите цвет канала", par.clrs[i]);
        if (newCol != null) {
            par.clrs[i] = newCol;
    ...
    private void btAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btAddActionPerformed
        ...
        if (cchan == 0) {
            JOptionPane.showMessageDialog(null, "В указанной установке нет каналов дефектоскопии", "Ошибка", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        par.cahsnIds = Arrays.copyOf(par.cahsnIds, i + 1);
        par.cahsnIds[i] = idChan;
        par.devIds = Arrays.copyOf(par.devIds, i + 1);
        par.devIds[i] = idUnit;
        par.clrs = Arrays.copyOf(par.clrs, i + 1);
        par.clrs[i] = Color.BLACK;
        mod.addElement(new ListGrItem(par.names[i], par.clrs[i]));
    ...
    private void btRemoveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btRemoveActionPerformed
        ...
        for (int j = i; j < par.names.length - 1; j++) {
            ...
            par.cahsnIds[j] = par.cahsnIds[j + 1];
            par.clrs[j] = par.clrs[j + 1];
        }
        i = par.names.length - 1;
        par.names = Arrays.copyOf(par.names, i);
        par.devIds = Arrays.copyOf(par.devIds, i);
        par.cahsnIds = Arrays.copyOf(par.cahsnIds, i);
        par.clrs = Arrays.copyOf(par.clrs, i);
